...
odoo.define('pos_multi_product_uom.main', function (require) {
  ...
  pos_model.Orderline=pos_model.Orderline.extend({
    init_from_JSON: function(json) {
      ...
      this.new_price = json.new_price;
        this.uom_after_promotion=json.uom_after_promotion;
    },
    get_unit: function(){
    ...
    export_as_JSON: function() {
      ...
      pack_lot_ids.new_price = this.new_price;
      return pack_lot_ids;
    },
  });
  ...
  class UnitSelectionPopupWidget extends AbstractAwaitablePopup {
    ...
    click_confirm(event){
      ...
      self.props.orderline.set_new_price(price);
       console.log(self.props.orderline.descuento_pdf)
       if (self.props.orderline.descuento_pdf!=0)
         {
            self.props.orderline.uom_after_promotion=true;
         }
      this.cancel();
    }     
  }
        ...
        if (event && event.target && $(event.target).attr('class') == "fa fa-balance-scale"){
          var categ=[];
            for(var key in self.env.pos.units_by_id){
              if(self.env.pos.units_by_id[key].category_id[1]===self.props.line.get_unit().category_id[1] && self.env.pos.units_by_id[key].active){
