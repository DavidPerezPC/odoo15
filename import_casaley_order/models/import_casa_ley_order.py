...
class CasaLeyImport(models.Model):
    ...
    def validate(self):
        ...
        data_file = io.StringIO(txt_data.decode("latin-1"))
        ...
        for rec in archive_lines:
            ...
            if sale_order not in sale_orders:
                duplicate_folio = self.env['sale.order'].search([('client_order_ref','=',sale_order)],limit=1)        
                if duplicate_folio:
                    duplicate_orders.append(duplicate_folio.client_order_ref) 
        ...
        if duplicate_orders:
            return {
                        ...
                        'params': {
                            'type': 'warning',
                            'message': _('Existen pedidos que ya estan en el sistema {}'.format(duplicate_orders)),
    ...
    def create_orders(self):        
        ...
        for rec in self.sale_order_ids:            
            ...
            if not warehouse:
                warehouse = rec.partner_id.user_id.property_warehouse_id.id
                
            duplicate_order = self.env['sale.order'].search([('x_order_reference','=',rec.name),('x_order_reference_date','=',rec.date_order),('partner_id','=',rec.partner_id.id)])
            if len(duplicate_order) < 2:
                data = {
                ...
                success_orders.append(rec)
            else:
                duplicate_orders.append(rec)
        
        duplicate_folios = ''
        success_folios = ''
        if duplicate_orders and not success_orders:            
            for order in duplicate_orders:
                duplicate_folios += ',' + order.name
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'type': 'warning',
                    'message': _('Existen pedidos duplicados [{}]'.format(duplicate_folios[1:])),
                    'next': {'type': 'ir.actions.act_window_close'},
                }
            }
        elif duplicate_orders and success_orders:
            for order in duplicate_orders:
                duplicate_folios += ',' + order.name
            for order in success_orders:
                success_folios = ',' + order.name
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'type': 'warning',
                    'message': _('Existen pedidos duplicados [{}], pero los folios [{}] fueron cargados exitosamente'.format(duplicate_folios[1:], success_folios[1:])),
                    'next': {'type': 'ir.actions.act_window_close'},
                }
            }        
        else:       
            return {
                ...
                'params': {
                    'type': 'success',
                    'message': _('Se cargaron exitosamente las ordenes'),
                    'next': {'type': 'ir.actions.act_window_close'},
                }
            }
