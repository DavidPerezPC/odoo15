...
class CouponReward(models.Model):
    ...
    def name_get(self):
        ...
        for reward in self:
            ...
            elif reward.reward_type == 'multi_discount':
                if reward.discount_type == 'percentage':
                    reward_percentage = str(reward.discount_percentage)
                    if reward.discount_apply_on == 'on_order':
                        reward_string = _("%s%% discount on total amount", reward_percentage)
    ...
    def _filter_not_ordered_reward_programs(self, order):
        ...
        for program in self:
            ...
            if program.reward_type == 'discount' and program.discount_apply_on == 'specific_products' and \
                ...
                continue
            if program.reward_type == 'multi_discount' and program.discount_apply_on == 'specific_products' and \
                    not order.order_line.filtered(
                        lambda line: line.product_id in program.discount_specific_product_ids):
                continue
            programs |= program
    ...
    def _custom_create_new_no_code_promo_reward_lines(self):
        ...
        for program in programs:
            ...
            # elif program.discount_line_product_id.id not in self.order_line.mapped('product_id').ids:
                ...
                order_lines = (self.order_line - self._get_reward_lines()).filtered(lambda x: program._get_valid_products(x.product_id) and self._is_quantity_valid(x.product_id.id,program.rule_min_quantity))
                if order_lines:
                ...
                for key in groupby(sorted(order_lines, key=lambda l: l.product_id.id), key=lambda l: l.product_id.id):
                    ...
                    if self._is_quantity_valid(key[0],program.rule_min_quantity):
    ...
    def _get_custom_reward_values_multi_discount(self, program):
        ...
        for line in order.order_line:
            if order._is_valid_product(program, line) and not line.is_reward_line:
                ...
                price_with_discount = line.price_unit * line.product_uom_qty - discount_amount
                ...
                additional_dicount = (price_with_discount - price_with_additional_discount) * 100 / (line.price_unit * line.product_uom_qty) 
                discount_sum = line.discount_rate + additional_dicount
                line.write({'discount': discount_sum})
                line.write({'discount_promotions': additional_dicount})
        return line
