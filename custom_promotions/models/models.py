...
class SaleOrder(models.Model):
    ...
    def _get_uom_quantity_lines(self, order_lines, rule_min_quantity):
        qty = 0
        for line in order_lines:
            ...
            qty += product_qty
        return qty // rule_min_quantity

    def _is_quantity_valid(self, product_id, rule_min_quantity):
        order_lines = self.order_line.filtered(lambda x: x.product_id.id == product_id and not x.is_reward_line)
        if self._get_uom_quantity_lines(order_lines, rule_min_quantity) >= 1:
            return True
        else:
            return False 
    ...
    def _get_custom_reward_values_product(self, program, program_action, product_id):
            ...
            if program._get_valid_products(program.reward_product_id):
                ...
                reward_qty = int(self._get_uom_quantity_lines(order_lines,program.rule_min_quantity))*program.reward_product_quantity
