...
class PosAutorizacion(models.Model):
    ...
    pricelist = fields.Float('pricelist', digits=(12, 1))
    priceproduct = fields.Float('priceproduct', digits=(12, 1))
    ruta = fields.Char(string='Ruta')
    fch_autorizacion = fields.Datetime(string='Fecha autorización')
    ...
    def aprobar(self):
        ...
        self.fch_autorizacion = fields.Datetime.now()
...
class PosAutorizacionLine(models.Model):
    ...
    unitario = fields.Float(string='Precio unitario')
    descuento = fields.Float(string='Desc.%')
    unitario_pdf = fields.Float(string='Precio unitario')
    descuento_pdf = fields.Float(string='Desc.%')
    ...
    def _onchange_qty(self):
        if self.product_id:
            ...
            price = round((self.unitario_pdf * (1 - (self.descuento_pdf or 0.0) / 100.0)), 6)
            ...
            if self.product_id.taxes_id:
                taxes = self.product_id.taxes_id.compute_all(
                    price,
                    self.autorizacion_id.pricelist_id.currency_id,
                    self.cantidad,
