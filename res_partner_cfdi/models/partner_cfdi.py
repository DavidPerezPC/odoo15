        ...
        for rec in self:
            ...
            if rec.partner_shipping_id:
                if rec.partner_shipping_id.x_l10n_mx_edi_payment_method_id:
                    return rec.partner_shipping_id.x_l10n_mx_edi_payment_method_id
                else:
                    return lambda self: self.env.ref('l10n_mx_edi.payment_method_otros', raise_if_not_found=False)
            else:
                if rec.partner_id.x_l10n_mx_edi_payment_method_id:
                    return rec.partner_id.x_l10n_mx_edi_payment_method_id
                else:
                    return lambda self: self.env.ref('l10n_mx_edi.payment_method_otros', raise_if_not_found=False)
